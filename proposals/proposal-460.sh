...
dfx canister --network ic call dwv6s-6aaaa-aaaaq-aacta-cai manage_neuron '(
  record {
    ...
    command = opt variant {
      MakeProposal = record {
        title = "Add admin_set_svm_providers to set SVM providers on the one_bridge_canister canister";
        ...
        summary = "Adding a new generic function to set SVM providers on the one_bridge_canister canister.";
        action = opt variant {
            AddGenericNervousSystemFunction = record {
                id = 1_302 : nat64;
                name = "Set SVM providers on the one_bridge_canister canister";
                description = opt "Set SVM providers on the one_bridge_canister canister.";
                function_type = opt variant {
                    GenericNervousSystemFunction = record {
                        ...
                        validator_method_name = opt "validate_admin_set_svm_providers";
                        target_method_name = opt "admin_set_svm_providers";
                    }
                };
            }
        };
