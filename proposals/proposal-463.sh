...
dfx canister --network ic call dwv6s-6aaaa-aaaaq-aacta-cai manage_neuron '(
  record {
    ...
    command = opt variant {
      MakeProposal = record {
        title = "Add admin_add_bridges to add sub bridges on the one_bridge_canister canister";
        ...
        summary = "Adding a new generic function to add sub bridges on the one_bridge_canister canister.";
        action = opt variant {
            AddGenericNervousSystemFunction = record {
                id = 1_305 : nat64;
                name = "Add sub bridges on the one_bridge_canister canister";
                description = opt "Add sub bridges on the one_bridge_canister canister.";
                function_type = opt variant {
                    GenericNervousSystemFunction = record {
                        ...
                        validator_method_name = opt "validate_admin_add_bridges";
                        target_method_name = opt "admin_add_bridges";
                    }
                };
            }
        };
