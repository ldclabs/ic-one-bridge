...
dfx canister --network ic call dwv6s-6aaaa-aaaaq-aacta-cai manage_neuron '(
  record {
    ...
    command = opt variant {
      MakeProposal = record {
        title = "Add admin_collect_fees to collect bridge fees on the one_bridge_canister canister";
        ...
        summary = "Adding a new generic function to collect bridge fees on the one_bridge_canister canister. It transfers collected bridge fees to a recipient, and cannot send more than the fees the canister has collected and not yet withdrawn.";
        action = opt variant {
            AddGenericNervousSystemFunction = record {
                id = 1_307 : nat64;
                name = "Collect bridge fees on the one_bridge_canister canister";
                description = opt "Collect bridge fees on the one_bridge_canister canister.";
                function_type = opt variant {
                    GenericNervousSystemFunction = record {
                        ...
                        validator_method_name = opt "validate_admin_collect_fees";
                        target_method_name = opt "admin_collect_fees";
                    }
                };
            }
        };
