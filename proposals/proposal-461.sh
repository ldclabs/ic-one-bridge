...
dfx canister --network ic call dwv6s-6aaaa-aaaaq-aacta-cai manage_neuron '(
  record {
    ...
    command = opt variant {
      MakeProposal = record {
        title = "Add admin_add_evm_contract to add EVM contract on the one_bridge_canister canister";
        ...
        summary = "Adding a new generic function to add EVM contract on the one_bridge_canister canister.";
        action = opt variant {
            AddGenericNervousSystemFunction = record {
                id = 1_303 : nat64;
                name = "Add EVM contract on the one_bridge_canister canister";
                description = opt "Add EVM contract on the one_bridge_canister canister.";
                function_type = opt variant {
                    GenericNervousSystemFunction = record {
                        ...
                        validator_method_name = opt "validate_admin_add_evm_contract";
                        target_method_name = opt "admin_add_evm_contract";
                    }
                };
            }
        };
