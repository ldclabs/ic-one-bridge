...
thread_local! { static TRAP_AFTER_SIGNATURE: Cell<bool> = const { Cell::new(false) }; }
...
#[ic_cdk::update(guard = "controller", hidden = true)]
fn test_trap_after_signature(enabled: bool) {
    TRAP_AFTER_SIGNATURE.set(enabled);
}

pub fn after_signature_reply() {
    if TRAP_AFTER_SIGNATURE.get() {
        ic_cdk::trap("PocketIC: trap after signature reply, before recording signed transaction");
    }
}
